...
##### 完整流程（前端 → 后端 → 第三方 API → 前端 → HTTP 生成试题）

1. **用户点击生成题目按钮**

2. **前端发送验证检查请求**：
   - 前端通过用户WebSocket发送 `check_qq_verify` 请求，包含用户QQ号

3. **后端检查验证状态**：
   - 检查是否启用QQ验证功能
   - 检查用户是否在白名单中
   - 检查用户是否已验证且在有效期内（24小时）
   - 如果以上任一条件满足，直接返回 `need_verify: false`

4. **后端向第三方客户端发送验证询问**（如果需要验证）：
   - 服务端向第三方客户端发送 `qq_verify_check` 请求
   - 第三方客户端返回 `qq_verify_check_response`，包含 `need_verify` 字段

5. **后端返回检查结果给前端**：
   - 如果第三方返回 `need_verify: true`：
     - 后端生成验证内容（12位随机字符串）
     - 后端向第三方发送 `qq_verify_request` 启动验证
     - 后端返回 `qq_verify_check_result` 给前端，包含验证内容和引导信息
   - 如果第三方返回 `need_verify: false`：
     - 后端返回 `qq_verify_check_result` 给前端，`need_verify: false`

6. **前端根据响应处理**：
   - 如果返回 `need_verify: false`，前端调用 HTTP `/api/generate` 接口生成试题
   - 如果返回 `need_verify: true`，前端弹出验证窗口，显示验证引导和加载动画（文字显示"等待进行验证"）

7. **第三方客户端处理验证**：
   - 第三方客户端在用户端执行验证操作
   - 验证完成后，第三方发送 `qq_verify_response` 给服务端

8. **后端推送验证结果给前端**（通过用户WebSocket `qq_verify_result` 消息）
...
9. **前端根据验证结果处理**：
   ...
   - **错误（error）**：关闭验证窗口，提示用户"验证系统异常"，返回生成题目页面

10. **第三方客户端超时处理**：
    - 第三方客户端应实现自动超时处理机制，在用户未能手动完成验证时，在2分钟后自动发送验证失败消息至服务端

11. **前端 WebSocket 断开监控**：
    - 前端在显示验证窗口后启动 10 秒断开监控
    - 如果 WebSocket 断开且 10 秒内未重连，提示"验证系统连接异常，请重新验证"
